...
const AdminMenuScreen = () => {
                ...
                { text: "Cancelar", style: "cancel" },
                {
                    ...
                    onPress: async () => {
                        try {
                            ...
                            // Volver a cargar los datos según el filtro actual para reflejar el cambio
    ...
    // --- CAMBIO CLAVE 2: Actualizar la función de filtrado con el estado de carga ---
    const handleFilterChange = async (newFilter) => {
        ...
        try {
            ...
            if (response.data.tipo === "SUCCESS") {
                ...
                setMenuItems([]) // Limpiar en caso de error para no mostrar datos viejos
            }
        } catch (error) {
            console.error("Error al filtrar productos:", error)
            Alert.alert("Error", "No se pudieron cargar los productos.")
        } finally {
            setLoading(false) // Desactivar el indicador de carga en cualquier caso
        }
    }
    ...
    // Actualizar el efecto de foco con el estado de carga 
    useFocusEffect(
        React.useCallback(() => {
            const fetchMenu = async () => {
                setLoading(true);
                try {
                    // Carga los productos según el filtro que ya estaba seleccionado
        ...
        if (loading) {
            return (
                <View style={styles.centered}>
                    ...
                    <Text style={{ color: theme.textColor, marginTop: 10 }}>Cargando productos...</Text>
                </View>
            )
        }
        ...
        if (productsFiltered.length === 0) {
            return (
                <View style={styles.emptyContainer}>
                    <MaterialCommunityIcons name="food" size={rw(20)} color="#BACA16" />
                    <Text style={[styles.emptyText, { color: theme.textColor }]}>
                        {search ? 'No se encontraron resultados' : 'No hay productos disponibles'}
                    </Text>
                </View>
            )
        }
